...
describe file('/etc/ssh/sshd_config') do
  ...
  its('content') { should match %r{/^AuthorizedKeysFile %h/.ssh/authorized_keys/} }
  ...
  its('content') { should match %r{/^HostKey /etc/ssh/ssh_host_rsa_key/} }
  its('content') { should match %r{/^HostKey /etc/ssh/ssh_host_ecdsa_key/} }
  its('content') { should match %r{/^HostKey /etc/ssh/ssh_host_ed25519_key/} }
  its('content') { should match /^LogLevel VERBOSE/ }
  its('content') { should match /^PasswordAuthentication yes/ }
  its('content') { should match /^Protocol 2/ }
  its('content') { should match %r{/^RevokedKeys /etc/ssh/revoked_keys/} }
  its('content') { should match %r{/^Subsystem sftp /usr/libexec/openssh/sftp-server/} }
  its('content') { should match /^SyslogFacility AUTHPRIV/ }
  its('content') { should match %r{/^TrustedUserCAKeys /etc/ssh/ca_keys/} }
